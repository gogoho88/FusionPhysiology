...
class GPT(nn.Module):
    ...
    def forward(self, video_tensor, radar_tensor):
        """
        Args:
            video_tensor (tensor): B, C, T (Height ,Width projected)
            radar_tensor (tensor): B, C, T (Freq. projected)
        """
        B, C, T = video_tensor.shape
        
        # forward the image model for token embeddings
        video_tensor = video_tensor.view(B, self.seq_len, -1, T)
        radar_tensor = radar_tensor.view(B, self.seq_len, -1, T)

        # pad token embeddings along number of tokens dimension
        token_embeddings = torch.cat([video_tensor, radar_tensor], dim=1).permute(0,1,3,2).contiguous()
        token_embeddings = token_embeddings.view(B, -1, self.n_embd) # (B, an * T, C)
        ...
        image_tensor_out = x[:, 0, :, :].contiguous()
        lidar_tensor_out = x[:, 1, :, :].contiguous()
